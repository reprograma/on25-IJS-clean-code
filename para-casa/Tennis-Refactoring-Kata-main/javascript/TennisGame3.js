class TennisGame3{
    ...
    getPointsName(){
        ...
        return (this.player1Point == this.player2Point) ? result + "-All" : result + "-" + namePoints[this.player2Point];
    }

    getPlayersName(){
        const resultName = this.player1Point > this.player2Point ? this.player1Name : this.player2Name;
        return resultName;
    }
    ...
    getScore() {
        if ((this.player1Point < 4 && this.player2Point < 4) && (this.player1Point + this.player2Point < 6)) {
            ...
            if (this.player1Point == this.player2Point)
                return "Deuce";
            return (((this.player1Point - this.player2Point) * (this.player1Point - this.player2Point)) == 1) ? "Advantage " + this.getPlayersName() : "Win for " + this.getPlayersName();

        }
    };

    wonPoint(playerName) {
        const point = playerName == "player1" ? this.player1Point += 1 : this.player2Point += 1;
        return point;
    }
